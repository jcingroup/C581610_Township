...
//ajax--
const apiPath: string = gb_approot + 'api/Editor_L2';
const apiDetailPath: string = gb_approot + 'api/Editor_L3';
...
export const ajaxGridDetailItem = (main_id: number) => {
    return dispatch => {
        return callGet(apiDetailPath + '/GetL3List', { main_id: main_id })
            .done((data, textStatus, jqXHRdata) => {
                dispatch(setInputValue(ac_type_comm.chg_fld_val, "Editor_L3", data));
            })
    }
}
