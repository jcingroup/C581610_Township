...
{
    public class CommunityBannerController : ajaxApi<Community_Banner>
    {
        public async Task<IHttpActionResult> Get(int id)
        {
            using (db0 = getDB0())
            {
                Community_Banner item = await db0.Community_Banner.FindAsync(id);
                var r = new ResultInfo<Community_Banner>() { data = item };
                return Ok(r);
            }
        }
        public async Task<IHttpActionResult> Get([FromUri]queryParam q)
        {
            ...
            var resultItems = await result
                ...
                .Select(x => new { x.community_banner_id, x.title, x.start_date, x.end_date, community_name = x.Community.community_name, x.state })
                .Skip(startRecord)
                .Take(defPageSize)
                .ToListAsync();
        ...
        public async Task<IHttpActionResult> Put([FromBody]putBodyParam param)
        {
            ...
            try
            {
                ...
                item = await db0.Community_Banner.FindAsync(param.id);
                var md = param.md;
        ...
        public async Task<IHttpActionResult> Post([FromBody]Community_Banner md)
        {
            md.community_banner_id = GetNewId(CodeTable.Base);

            r = new ResultInfo<Community_Banner>();
            ...
            try
            {
                ...
                db0.Community_Banner.Add(md);
                await db0.SaveChangesAsync();

                r.result = true;
                r.id = md.community_banner_id;
                return Ok(r);
                #endregion
            }
        ...
        public async Task<IHttpActionResult> Delete([FromBody]delParam param)
        {
            try
            {
                ...
                if (item != null)
                {
                    db0.Community_Banner.Remove(item);
                    await db0.SaveChangesAsync();
                    r.result = true;
                    return Ok(r);
                }
        ...
        public class putBodyParam
        {
            ...
            public Community_Banner md { get; set; }
        }
